...
BEGIN
  ...
  IF v_policy.emergency_pause THEN
    ...
    v_rules := COALESCE(v_rules, '[]'::jsonb) ||
      jsonb_build_array(jsonb_build_object('rule', 'emergency_pause', 'result', 'blocked'));
    v_reason := array_to_string(v_reasons, ' ');
    v_reason := CASE
      WHEN v_reason IS NULL OR v_reason = '' THEN 'Emergency pause is enabled.'
      ELSE v_reason || ' / Emergency pause is enabled.'
    END;
